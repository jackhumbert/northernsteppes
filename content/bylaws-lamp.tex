...
\title{% 
    Northern Steppes Bylaws \\
    \large 2025 Rewrite Revision 1
}
...
\usepackage[commandnameprefix=always, markup=bfit, addedmarkup=colored, deletedmarkup=sout, commentmarkup=footnote, authormarkup=none, defaultcolor=BrickRed]{changes}
...
% \newcommand{\tinycolorbox}[2]{\tikz[baseline=(a.base),inner sep=0pt]\node[fill=#1](a){#2};}

% \setaddedmarkup{\tinycolorbox{ForestGreen}{#1}\ }
\setaddedmarkup{\color{ForestGreen}{#1}\ }

% \setcommentmarkup{\footnote{\parbox[t]{\linewidth}{#1}}}
\setcommentmarkup{\footnote{#1}}
...
\setcounter{FileDepth}{-5}       % Split \HTML\ files at sections
...
% \section{Definitions}
\section{Definitions\chcomment{Completely new article}}
\begin{level}
    \item \textbf{Realm}. This organization, previously known as a \emph{chapter}\chcomment{All instances of ``chapter'' have been replaced with ``realm''}.
...
\begin{level}
    ...
    \begin{level}
        ...
        \begin{level}
            \item The healer cannot heal \chreplaced[comment=More inclusive language]{themselves}{her/himself} or their own equipment.
        ...
        \end{level}
    \end{level}
    \item \chadded{All leadership, members, and participants must abide by the Code of Conduct established by the Council.}
\end{level}

% friend of the steppes tier? support financially
% local clause for membership?
% regularly attend practices?

% \section{Membership}\label{membership}
\section{Membership\chcomment{Completely rewritten article---main changes are highlighted}}\label{membership}
\begin{level}
    \item \textbf{Benefits}. While anyone may attend events and use loaner gear, participants may annually register as a \emph{Member} of Northern Steppes and be granted the following benefits:
    \begin{level}
        \item \chadded{Priority use of loaner gear.}
        \item \chadded{Use of any Council-specified, Member-only loaner gear.}
        \item Voting rights during the annual meeting and other voting events.
        \item \chadded[comment=Removed GOTC day option]{Entrance to all Northern Steppes build days, if not free.}
        % belt flag?
    \end{level}
    \item \textbf{Registration}. \chadded{Members may not register in the period between the closing of bylaw amendments mentioned in Article~\expandafter{\ref{amendments}}
        and the end-of-voting during the annual meeting}. Otherwise, registration happens as soon as dues are paid.
    \item \textbf{Term}. The Council will decide to implement monthly or yearly terms. Either term expires at the end-of-voting at the annual meeting. 
    \item \textbf{Dues}. The Council will determine the cost of dues, and announce it at the annual meeting. 
    \begin{level}
        \item \chadded{To encourage registering at the beginning of a season, the Council may decide to discount the dues prior to a selected date, such as May 1st.} 
        \item If dues are unaffordable by someone wanting to become a Member, the Council may approve an explicit amount of volunteering, completed prior to their registration, in lieu of dues. (list specific examples)
    \end{level}
...
\section{Leadership\chcomment{Completely rewritten article---main changes are highlighted}}\label{leadership}
\begin{level}
    ...
    \begin{level}
        \item \chadded{\textbf{Voting}. The Council may vote on day-to-day matters for which there is no clear consensus.}
        \begin{level}
        ...
        \end{level}
        \item \chadded{\textbf{Capabilities}}. A \emph{Simple Majority} is required unless otherwise specified:
        \begin{level}
        ...
        \end{level}
        \item \textbf{Removal}. If a petition signed by \chreplaced[comment=Revised quorum definition accomplishes the same goal]{a quorum of realm members}{Abstained signatures are only considered ``nay'' in this scenario; this is in exception to Article VI Section 1.1.} to impeach an officer is presented to the Council, the Council should:
        \begin{level}
            \item Give that officer one month to turn over responsibilities to another officer, \chadded{at which point that office is considered vacant, and may be appointed via Section~\ref{officer_replacement}}\label{officer_yield}
            \item \chadded{\emph{OR} vote to remove the officer immediately via Section~\expandafter{\ref{officer_removal}}. If the vote fails, Section~\expandafter{\ref{officer_yield}} must be followed.}
        \end{level}
    ...
    \item \textbf{President}
        ...
        \end{level}
        \item \textbf{Secretary}\chcomment{Created from Quartermaster's responsibilities}
        \begin{level}
            ...
            \begin{level}
                \item \chadded{Keep a list of people that are able to operate Troll and make Leadership and special event staff aware of them.}
            \end{level}
            \item Take notes/minutes during meetings.
            \item \chadded{Store/update any standard operating procedures.}
            \item Establish standard operating procedures to:
            \begin{level}
        ...
        \begin{level}
            ...
            \item \chadded{Coordinate a presence at other events, and serve as/delegate a campmaster.}
            \item \chadded{Establish standard operating procedures for event-specific tasks, such as:}
            \begin{level}
            ...
            \end{level}
        \end{level}
        \item \textbf{Safety Officer}\chcomment{Previously known as Warden}
        \begin{level}
            ...
            \item \chadded{Establish standard operating procedures to:}
            \begin{level}
...
\begin{level}
    ...
    \begin{level}
        ...
        \end{level}
    \end{level}
    \item \chadded{\textbf{Quartermaster}}. Safety Officer if vacant. Has the following responsibilities:
    \begin{level}
    ...
    \end{level}
    \item \chadded{\textbf{Battlemaster}}. Event Officer if vacant. Has the following responsibilities:
    \begin{level}
    ...
    \begin{level}
        ...
        \item \chadded{Have access to and regularly check the realm email account.}
    ...
    \begin{level}
        ...
        \item \chadded{In collaboration with the President, communicate voting efforts.}
    \end{level}
...
% virtual meetings/broadcast

\section{Meetings and Quorum}\label{annual_meeting}
\begin{level} 
    \item \chadded{A realm member meeting may be scheduled at any time, given that members are notified at least four (4) weeks prior to the meeting.}
    ...
    \end{level}
    \item During any member meeting, \chreplaced[comment={This wording was confusing, and as our membership grows, it may be impossible to get this many people. 50\% + 1 is safer, more common, and will prevent minority rule.}]{the quorum shall consist of 50\% of members plus one (1)}{Quorum is defined as 75\% of the subject in question. This applies to all votes and attendance of meetings. Regarding votes, a vote of “abstain” is not included in the total count of those present for that issue}.
\end{level}
...
\begin{level} 
    ...
    \item \chadded{Amendments should be read and discussed during a member meeting prior to voting, if quorum is present.}
    \item \chreplaced[comment={This wording was unclear, as quorum is usually applied to the number of votes needed for a valid vote, not pass/fail status.}]{A \emph{Simple Majority} is required for an amendment to pass.}{Quorum of those in attendance is necessary for them to pass.}
\end{level}
