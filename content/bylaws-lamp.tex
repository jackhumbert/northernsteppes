...
\usepackage[markup=bfit, addedmarkup=colored, deletedmarkup=sout, commentmarkup=footnote, authormarkup=none, defaultcolor=BrickRed]{changes}
...
\section{Definitions\comment{Completely new article}}
\begin{level}
    \item \textbf{Realm}. This organization, previously known as a \emph{chapter}\comment{All instances of ``chapter'' have been replaced with ``realm''}.
...
\begin{level}
    ...
    \begin{level}
        ...
        \begin{level}
            \item The healer cannot heal \replaced[comment=More inclusive language]{themself}{her/himself} or their own equipment.
        ...
        \end{level}
    \end{level}
    \item \added{All leadership, members, and participants must abide by the Code of Conduct established by the Council.}
\end{level}

\section{Membership\comment{Completely rewritten article---main changes are highlighted}}\label{membership}
\begin{level}
    ...
    \begin{level}
        \item \added{Priority use of loaner gear.}
        \item \added{Use of any Council-specified, Member-only loaner gear.}
        \item Voting rights during the annual meeting and other voting events.
        \item \added{Entrance to all Northern Steppes non-overnight events, such as:}
        \begin{level}
        ...
        \end{level}
    \end{level}
    \item \textbf{Registration}. \added{Members may not register in the period between the closing of bylaw amendments mentioned in Article~\expandafter{\ref{amendments}}
    ...
    \begin{level}
        \item \added[comment=Discount and/or standard rate may be adjusted---we could remove it from the bylaws \& let the Council decide it on a yearly basis.]{To encourage registering at the beginning of a season, any registration prior to May 1st will receive a discount of \$5.} 
...
\section{Leadership\comment{Completely rewritten article---main changes are highlighted}}\label{leadership}
\begin{level}
    ...
    \begin{level}
        \item \added{\textbf{Voting}. The Council may vote on day-to-day matters for which there is no clear consensus.}
        \begin{level}
        ...
        \end{level}
        \item \added{\textbf{Capabilities}}. A \emph{Simple Majority} is required unless otherwise specified:
        \begin{level}
        ...
        \end{level}
        \item \textbf{Removal}. If a petition signed by \replaced[comment=Revised quorum definition accomplishes the same goal]{a quorum of realm members}{Abstained signatures are only considered ``nay'' in this scenario; this is in exception to Article VI Section 1.1.} to impeach an officer is presented to the Council, the Council should:
        \begin{level}
            \item Give that officer one month to turn over responsibilities to another officer, \added{at which point that office is considered vacant, and may be appointed via Section~\ref{officer_replacement}}\label{officer_yield}
            \item \added{\emph{OR} vote to remove the officer immediately via Section~\expandafter{\ref{officer_removal}}. If the vote fails, Section~\expandafter{\ref{officer_yield}} must be followed.}
        \end{level}
    ...
    \item \textbf{President}
        ...
        \end{level}
        \item \textbf{Secretary}\comment{Created from Quartermaster's responsibilities}
        \begin{level}
            ...
            \begin{level}
                \item \added{Keep a list of people that are able to operate Troll and make Leadership aware of them.}
            \end{level}
            \item Take notes/minutes during meetings.
            \item \added{Store/update any standard operating procedures.}
            \item Establish standard operating procedures to:
            \begin{level}
        ...
        \begin{level}
            ...
            \item \added{Coordinate a presence at other events, and serve as/delegate a campmaster.}
            \item \added{Establish standard operating procedures for event-specific tasks, such as:}
            \begin{level}
            ...
            \end{level}
        \end{level}
        \item \textbf{Safety Officer}\comment{Previously known as Warden}
        \begin{level}
            ...
            \item \added{Establish standard operating procedures to:}
            \begin{level}
...
\begin{level}
    ...
    \begin{level}
        ...
        \end{level}
    \end{level}
    \item \added{\textbf{Quartermaster}}. Safety Officer if vacant. Has the following responsibilities:
    \begin{level}
    ...
    \end{level}
    \item \added{\textbf{Battlemaster}}. Event Officer if vacant. Has the following responsibilities:
    \begin{level}
    ...
    \begin{level}
        ...
        \item \added{Have access to and regularly check the realm email account.}
    ...
    \begin{level}
        ...
        \item \added{In collaboration with the President, communicate voting efforts.}
    \end{level}
...
\begin{level} 
    \item \added{A realm member meeting may be scheduled at any time, given that members are notified at least four (4) weeks prior to the meeting.}
    ...
    \end{level}
    \item During any member meeting, \replaced[comment={This wording was confusing, and as our membership grows, it may be impossible to get this many people. 50\% + 1 is safer, more common, and will prevent minority rule.}]{the quorum shall consist of 50\% of members plus one (1)}{Quorum is defined as 75\% of the subject in question. This applies to all votes and attendance of meetings. Regarding votes, a vote of “abstain” is not included in the total count of those present for that issue}.
\end{level}
...
\begin{level} 
    ...
    \item \added{Amendments should be read and discussed during a member meeting prior to voting, if quorum is present.}
    \item \replaced[comment={This wording was unclear, as quorum is usually applied to the number of votes needed for a valid vote, not pass/fail status.}]{A \emph{Simple Majority} is required for an amendment to pass.}{Quorum of those in attendance is necessary for them to pass.}
\end{level}
